...
ICudaEngine* createEngine_s(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt) {
    ...
    std::map<std::string, Weights> weightMap = loadWeights("../yolov5s_960n2.wts");
...
#ifdef SHOW
        std::string video_file_name = "/home/nvidia/tensorrtx/yolov5/video_pred/" + file_name;
        cv::VideoWriter writer(video_file_name, cv::VideoWriter::fourcc('m', 'p', '4', 'v'), 10, videoSize, true);
#endif
        cv::Mat frame;
        frame = capture.open(std::string(argv[2]) + "/" + file_name);
        if(!capture.isOpened())
        {
            printf("can not open ...\n");
            continue;
...
#ifdef TO_FILE
        std::ofstream video_out("/home/nvidia/" + file_name.substr(0, file_name.length() - 4) + ".txt");
#endif
           ...
           for (int row = 0; row < INPUT_H; ++row) {
            ...
            for (int b = 0; b < fcount; b++) {
                ...
                for (auto &re : res) {
                    ...
                    cv::Rect r = get_rect(frame_copy, re.bbox);
...
//                for (const auto& det_obj : detect_objects){
//                     std::cout << std::to_string(det_obj.rect.x)+"-"+std::to_string(det_obj.rect.y)+"-"+
//                                   std::to_string(det_obj.rect.width)+"-"+std::to_string(det_obj.rect.height)+"#"<<det_obj.confidence<<std::endl;
//                }

                Tracker.update(detect_objects, line_point, frame_copy);
                track_objects = Tracker.get_detect_obj();

//                for (const auto& det_obj : track_objects){
//                    std::cout << det_obj.track_id << "," << det_obj.rect.x << "," << det_obj.rect.y << "," << det_obj.rect.width << "," << det_obj.rect.height << "," << det_obj.confidence <<std::endl;
//                }

#ifdef TO_FILE
                for (const auto& det_obj : track_objects){
                    video_out << frame_id << "," << det_obj.track_id << "," << det_obj.rect.x << "," << det_obj.rect.y << "," << det_obj.rect.width << "," << det_obj.rect.height << "," << det_obj.confidence << "," << det_obj.class_id << ",1,1\n";
                }
#endif
                ...
                for (const auto& det_obj : track_objects){
                    ...
                    cv::rectangle(frame_copy, rec, color_map[color_id], 2);
                    cv::putText(frame_copy, classes[det_obj.class_id] + ":"+ std::to_string(det_obj.track_id), cv::Point(rec.x, rec.y - 1), cv::FONT_HERSHEY_PLAIN, 1.2, color_map[color_id], 2);
                }
                    cv::imshow("test", frame_copy);
                    cv::waitKey(8);
                    writer << frame_copy;
#endif
            }
